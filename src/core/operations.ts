...
const extract_facts: Operation = {
  name: 'extract_facts',
  description:
    'v0.31: extract personal-knowledge facts (events, preferences, commitments, beliefs) from a conversation turn into the per-source hot memory. Sanitizes turn_text via INJECTION_PATTERNS, calls Haiku to extract structured claims, drops any extracted claim carrying high-signal restricted data (payment-card number, SSN, or credential/secret) before persistence, runs the cosine fast-path + classifier dedup pipeline, INSERTs into facts. Returns counts by status. Skips extraction when the turn is dream-generated content (anti-loop).',
  params: {
...
const save_facts: Operation = {
  name: 'save_facts',
  description:
    'B7: insert PRE-EXTRACTED structured facts into per-source hot memory with ZERO server LLM. The deterministic sibling of extract_facts: the CALLER (the customer\'s own model) decides what is worth remembering and structures it; the server only validates, sanitizes (INJECTION_PATTERNS + 500-char cap), drops restricted-data claims, dedups, and inserts. Use this on the tenant plane where the chat gateway is intentionally unavailable. Each claim carries a REQUIRED provenance ("user_stated" for what the user actually said; "model_inferred" only for conservative, clearly-flagged inferences — never speculation). Dedup is two-layer and degrades gracefully: always-on pg_trgm / normalized-text, plus cosine when an embedding provider is configured. Rows are stamped source=mcp:save_facts, client_authored=true, provenance + confidence per row. A claim MAY carry supersedes (a prior fact id) to record a CORRECTION: the target row is expired and superseded_by-linked to the replacement (atomically on the insert path), so the old value stays in history and recall(supersessions:true) shows the chain. Corrections are DURABLE: a fence-backed target is struck in its markdown fence so the supersession survives rebuild/reconcile; when the fence cannot be struck, that claim\'s result discloses supersede_durable:false + supersede_reason — the correction applies now but the next reconcile of that page resurrects it. Returns {inserted, duplicate, dropped, superseded, fact_ids, results, dedup_mode}. `results` is the per-claim receipt — exactly one entry per input claim, ordered by `index` (position in the request array): {index, status: "inserted"|"duplicate"|"duplicate_superseded", fact_id} or {index, status: "dropped", category}. Use `results` (not a positional zip of fact_ids) to attribute outcomes to claims — fact_ids has no entry for dropped claims, so positional alignment breaks whenever dropped > 0. A superseding insert reports status "inserted" (it wrote a new row); the batch `superseded` counter reports the chain. Re-saving text that was previously CORRECTED AWAY (its row is expired with superseded_by set) does NOT re-mint: the receipt is {status: "duplicate_superseded", fact_id: <the LIVE HEAD of the correction chain>, superseded_from: <the matched expired row>}, counted under duplicate — an MCP retry or stale-export replay that crosses a correction can never silently resurrect the corrected-away claim. To deliberately re-assert it (the correction itself was wrong), carry supersedes:<live head id> — the claim then re-mints atomically with the chain intact; there is no separate force flag. The head id is ENFORCED: a supersedes naming any OTHER row (e.g. a stale correction batch replayed after a further correction landed) gets the same duplicate_superseded refusal, so a replayed correction can never resurrect a mid-chain claim beside the live head. Rows expired by forget/decay never block a re-save. Conceded edge: the tombstone match is EXACT text (normalized case/whitespace) — a PARAPHRASE of a corrected-away claim still mints as a new fact. A malformed claim rejects the WHOLE batch and names the failing index; a well-formed claim carrying high-signal restricted data (payment-card number, SSN, or credential/secret), OR one whose text is assistant-steering / setup instructions a client mislabeled as a user fact (a steering-document header label LEADING the claim — "Default behavior:"/"System prompt:"/"Custom instructions:" — combined with third-person "the user"/"the assistant" framing AND a directive commanding the assistant), is instead DROPPED from the batch — counted in `dropped` with its category ("payment_card" | "ssn" | "credential" | "instruction_shaped") reported honestly in `results`, never inserted, never logged or returned by value — while the rest of the batch proceeds; a dropped claim that carried supersedes additionally reports supersede_not_applied:true (the named correction was NOT applied). Merely naming a product tool id (save_facts, morning_brief, …) is NEVER by itself grounds to drop — engineer/PM memories about the product\'s own API are genuine memories and insert. First-person preferences ("when I ask you to save, round to whole dollars") are genuine memories and still insert, even when they name a tool.',
  params: {
    claims: {
      type: 'array',
      required: true,
      description:
        'Non-empty array of claim objects. Each object (strict — unknown keys rejected): claim (string, required, plain text, <=500 chars); provenance ("user_stated" | "model_inferred", REQUIRED, no default); kind ("fact" | "event" | "commitment" | "preference" | "belief", default "fact"); people (string[], surface forms as spoken, NOT slugs); entities (string[], non-person surface forms); date_context (string — resolve relative dates like "next Saturday" to a concrete date before sending); valid_from (string, YYYY-MM-DD or ISO — the SOURCE date this claim was true/stated, NOT today; set it when saving an imported or historical conversation so the fact reads and ages as of THEN; omit for something happening now; a garbage or future value is ignored and the row dates to now); confidence (number 0-1; default 1.0 for user_stated, hard-capped at 0.7 for model_inferred); supersedes (number — id of a prior fact this claim replaces/corrects; the target is expired + superseded_by-linked to the replacement, atomically on the insert path; omit for a plain save; also the escape hatch for re-asserting corrected-away text — see duplicate_superseded in the tool description).',
      items: { type: 'object' },
    },
  },
  mutating: true,
