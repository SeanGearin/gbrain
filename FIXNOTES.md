...
## fix/engine-creation-version — 2026-07-17 (creation-origin version rows: the time-travel first-window unlock)

The worker's E1 fix (`open_note_as_of`) refuses every as-of strictly before a page's earliest banked snapshot, because the engine banked versions ONLY on update-of-existing (`if (existing) createVersion` — import-file.ts markdown/code/`withImportTransaction` sites + revert + materialize): a page's creation state was never banked, and version rows carried no origin marker, so the wire could not distinguish a new page's genuinely-creation-holding earliest snapshot from a legacy page's gap-absorbing one. Honest, but it swallowed every genuinely-new page's creation→first-snapshot era.

The unlock (option (a) — creation-banked version row, chosen over a per-page "versioning-complete-since" watermark):

1. **Schema**: `page_versions.origin TEXT NOT NULL DEFAULT 'update'` (migration v116 `page_versions_origin_creation_marker` + inline in `src/schema.sql` / `pglite-schema.ts` / regenerated `schema-embedded.ts`). The DEFAULT is the factually-correct backfill for every legacy row — all were banked by update-of-existing, revert, or migrate-engine — so **no legacy page can ever carry a creation row**; the signal is opt-in per page and its absence reproduces today's honest refusal.
2. **Engine**: `createVersion(slug, {sourceId?, origin?})` stamps the row ('creation' only from callers that PROVED non-existence); `getVersions` gains an `id DESC` tie-break so same-instant rows (tx-frozen `now()`: a birth's creation row + the first materialize's bank) are wire-deterministic in reverse event order.
3. **Creation sites** bank the just-created state AFTER `putPage`, inside the creating call: `importFromContent` (the put_page op path), `importCodeFile`, `withImportTransaction` (markdown/image), and `ensureStub` (save_facts entity birth). Uninstrumented creators (synthesize, output writer, receipts, think, enrichment, split_entity target stubs, migrate-engine) fail SAFE: their pages have no creation row → the worker keeps refusing, exactly as today.

**Worker read contract** (what pro-time-travel should consume): `get_versions` rows now carry `origin`. `history_complete_from_creation = (oldest row's origin === 'creation')` — oldest = last row in the DESC wire order. When true, the creation→first-snapshot era is a BOUNDED window (recorded creation floor below, banked snapshot above) and may serve under the same lost-interior-rewrite fidelity caveat as every other bounded window; the creation row's own `compiled_truth` is the creation state, and on the import paths `snapshot_at == page.created_at` exactly (same-tx `now()`). Key on `origin`, NEVER on timestamp equality — PGLite's `withSourceScope` is a pass-through, so the ensureStub path's `snapshot_at` can trail `created_at` by call latency there. The signal claims a **recorded floor**, not write-path totality: versionless rewrites (direct `putPage`, `refreshPageBody`) remain the disclosed r2 residual.

Why (a) over (b): a watermark asserting "versioning complete since T" is a standing invariant every current AND future write path must maintain — and it is already false today (refreshPageBody, direct putPage callers). A creation row is a one-time append-only fact about one row that never rots, and it reuses the existing bounded-window serving + disclosure machinery unchanged.

Tests: `test/page-creation-versioning.test.ts` — red-first 0/7 pre-fix (birth mints ZERO rows: `Expected: 1, Received: 0`; `column "origin" does not exist`) → 7/7 post-fix (all four creation sites bank; `hadExisting=true` keeps the pre-update contract; uninstrumented-path page has no creation row; origin-less INSERT reads 'update' = the migration backfill). `test/facts-materialize-versioning.test.ts` updated for the birth chain (creation row + materialize bank, tied instants, oldest-is-creation) — 4/4.

Deploy: schema migration v116 (metadata-only ADD COLUMN, instant, idempotent). Box deploy is Sean-gated, as always.
