...
### 3. Test-infra (found while landing C1's suite): search-brain-pipeline's beforeAll was co-run-order roulette

Adding ANY serial suite whose teardown ends in `resetGateway()` could kill `test/search-brain-pipeline.test.ts` in combined runs (bun's file execution order is filename-dependent, not CLI order): its beforeAll ran `initSchema()` BEFORE configuring the gateway, the legacy 1536 preload only re-pins in a global beforeEach (which never fires before another file's beforeAll), so an empty slot built the v0.37 default `halfvec(1280)` schema, the `vector()`-only DIM introspection fell back to 1536, and the seed died in `halfvec.c` CheckExpectedDim — suite dead in co-runs, green alone. Receipted at the pristine branch tip with the branch's OWN compound-parity suite copied under a different filename: same kill, zero new content involved. Cure honors the preload's documented contract: configure the gateway FIRST in beforeAll, then initSchema (introspection stays as a cross-check). Full 10-file combined set 128/0 post-cure; the suite is order-independent now.
